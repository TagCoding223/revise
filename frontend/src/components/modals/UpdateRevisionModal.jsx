...
export default function UpdateRevisionModal({ isOpen, onClose, onUpdate, topicData }) {
  const [title, setTitle] = useState('');
  const [description, setDescription] = useState('');
  const [links, setLinks] = useState(['']);

  // Pre-fill data when the modal opens or topicData changes
  useEffect(() => {
    if (topicData) {
      setTitle(topicData.title || '');
      setDescription(topicData.description || '');
      // Handle legacy string link or new array of links
      if (Array.isArray(topicData.links) && topicData.links.length > 0) {
        setLinks(topicData.links);
      } else if (topicData.link) {
        setLinks([topicData.link]);
      } else {
        setLinks(['']);
      }
    }
  ...
  const handleSubmit = (e) => {
    ...
    onUpdate({ ...topicData, title, description, links: cleanedLinks });
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/50 dark:bg-black/60 backdrop-blur-sm">
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl w-full max-w-lg overflow-hidden border border-gray-100 dark:border-gray-700 animate-in fade-in zoom-in-95 duration-200">
        <div className="p-6">
          <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-6">Update Revision</h2>
